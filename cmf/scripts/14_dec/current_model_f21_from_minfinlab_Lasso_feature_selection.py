...
def calc_features(
    ...
    solusdt_trades: pd.DataFrame,
    ethusdt_trades: pd.DataFrame,
    btcusdt_trades: pd.DataFrame,
...
if __name__ == '__main__':
    ...
    # Separate trades by instrument
    solusdt_trades = agg_trades[agg_trades['instrument'] == 'SOLUSDT']
    ethusdt_trades = agg_trades[agg_trades['instrument'] == 'ETHUSDT']
    btcusdt_trades = agg_trades[agg_trades['instrument'] == 'BTCUSDT']
    ...
    X = calc_features(
        orderbook_solusdt,    # lobs
        agg_trades,           # agg_trades
        solusdt_trades,       # solusdt_trades
        ethusdt_trades,       # ethusdt_trades
        btcusdt_trades,       # btcusdt_trades
        orderbook_embedding,  # lobs_embedding
        target_data_solusdt   # target_data
        )
    ...
    for fold, (train_index, test_index) in enumerate(tscv.split(X)):
        ...
        # param_grid = {'alpha': [1e-4, 1e-3, 1e-2, 1e-1]} #strictly increasing cousin zero results after feature selection due to the fact that too small amount of features
